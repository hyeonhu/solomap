...
export default function AdminDashboardPage() {
  ...
  const needsCheck = mockEvents.filter((e) => e.status === 'needs_check').length
  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8 sm:grid-cols-4">
        {[
          { label: '전체 행사', value: mockEvents.length },
          { label: '게시 중', value: published },
          { label: '확인 필요', value: needsCheck },
          { label: '업체 수', value: mockOrganizers.length },
        ].map((s) => (
